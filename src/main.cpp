...
int64 nChainStartTime = 1373654820;
...
const string strMessageMagic = "federationcredit Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else
    {
        ...
        nSubsidy >>= (nHeight / 8000000); // federationcredit
    }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal federationcredit miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in federationcredit
    bool fStrictPayToScriptHash = true; // Always active in federationcredit
    ...
    // ppcoin: fees are not collected by miners as in federationcredit
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "federationcredit", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Set Phasers on Stun!";
        CTransaction txNew;
		...
		txNew.strTxComment = "text:federationcredit genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x7066f1fd6cdda6274df992ed323b01e97b85260d3d48b79fcef07028c92ccef1"));
...
unsigned char pchMessageStart[4] = { 0xcc, 0x17, 0x01, 0xdd };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("federationcreditMiner : proof-of-work not meeting target");

    //// debug print
    printf("federationcreditMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("federationcreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("federationcreditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadfederationcreditMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = 1;

void federationcreditMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("federationcredit-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running federationcreditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadfederationcreditMiner(void* parg)
{
    ...
    try
    {
        ...
        federationcreditMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadfederationcreditMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadfederationcreditMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadfederationcreditMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d federationcreditMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadfederationcreditMiner, pwallet))
                printf("Error: NewThread(ThreadfederationcreditMiner) failed\n");
            Sleep(10);
        }
    }
}

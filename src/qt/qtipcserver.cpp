...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "federationcredit:"))
        {
...
static void ipcThread(void* pArg)
{
    ...
    RenameThread("federationcredit-gui-ipc");
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any federationcredit: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one federationcredit instance is listening
        message_queue::remove(BITCOINURI_QUEUE_NAME);
        delete mq;
